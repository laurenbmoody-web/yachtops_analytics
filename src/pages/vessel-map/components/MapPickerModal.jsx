...
export default function MapPickerModal({ placingItem, placingDefect, placingStorage, initialScanId, onPlaced, onClose }) {
  useEffect(() => {
    ...
    <div className="vm-picker-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose?.(); }}>
      <div className="vm-picker-panel" role="dialog" aria-modal="true" aria-label="Pick a location on the map">
        ...
        <Suspense fallback={<div className="vm-picker-loading">Loading map…</div>}>
          <VesselMapPage embedded placingItem={placingItem} placingDefect={placingDefect} placingStorage={placingStorage} initialScanId={initialScanId} onPlaced={onPlaced} onClose={onClose} />
        </Suspense>
      </div>
    </div>,
    document.body,
