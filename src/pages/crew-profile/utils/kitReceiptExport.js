...
export const exportKitReceipt = async ({ crewName, vesselName, vessel, generatedAt, items, ackSig, returnSig }) => {
  ...
  // Vessel identity. The vessel name (plus registry details when known) anchors
  // this to a specific ship — it's a responsibility record. The generated date
  // sits in the footer, opposite the Cargo mark.
  ...
  // Generated date, bottom-right (opposite the Cargo mark).
  if (generatedAt) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(...MUTED);
    doc.text(`Generated ${generatedAt}`, pageW - M, pageH - 9, { align: 'right' });
  }
