...
// wardrobes garments live in on board; Guest = the cases guests' items travel
...
const WardrobeManagement = () => {
  ...
  // The two figures that matter on the hub: owner garments on board, and guests'
  // items on board. Everything else (laundry, value, wardrobe count) lives inside
  // the two worlds, not up here.
  const ownerCount = items.filter((i) => i.wardrobeId && !i.isArchivedFromToday).length;
  const guestCount = items.filter((i) => i.caseId && !i.isArchivedFromToday).length;
