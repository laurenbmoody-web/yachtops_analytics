...
// ── Item lifecycle cascade ─────────────────────────────────────────────────
// Single cascade: draft → ordered on per-supplier-group dispatch from
// SendToSupplierModal. Uses the RLS-safe .update().in() pattern (UPDATE
// policy only — no INSERT WITH CHECK evaluation, list_id stays on the
// existing row). Defensively scoped via a status filter on the WHERE
// clause so a race (another user flips an item mid-cascade) can't
// silently overwrite settled state.
//
// Phase 2 commit 1 originally added a Path 1 cascade (draft → to_order on
// board approval) too. Reverted in Phase 3 commit 1 — `to_order` was an
// artificial intermediate state with no real workflow meaning; items now
// stay at `draft` until physically dispatched.

// Cascade items → ORDERED. Caller passes the IDs of items just
// dispatched (typically the per-supplier-group items array from
// SendToSupplierModal's sendGroup flow). The .eq('status', 'draft')
// filter on the WHERE clause is defensive — only items currently in
// draft flip forward. Items already in a settled delivery state are
// not touched (guards against a race where a stew marks something
// received between dispatch start and cascade).
 ...
 */
export const createInventoryItemFromProvItem = async ({ provItem, categoryPath, storageLocations, locationName, qty, tenantId, userId, stockUnit = null, purchaseUnit = null, unitsPerPack = null }) => {
  if (!tenantId) return null;
  try {
    ...
    const { data, error } = await supabase
      ...
      ?.insert({
        ...
        // For a bulk line the caller passes the resolved inner stocking unit
        // (pack_unit ?? 'each'); the 'case' becomes purchase_unit below.
        unit: normalizeUnit(stockUnit || provItem?.unit) || 'each',
        purchase_unit: purchaseUnit ? normalizeUnit(purchaseUnit) : null,
        units_per_pack: Number(unitsPerPack) > 0 ? Number(unitsPerPack) : null,
 ...
 */
export const pushReceivedSplitsToInventory = async ({ inventoryItemId, splits, tenantId, provisioningItemId = null, listId = null, unit = null, size = null, purchaseUnit = null, unitsPerPack = null }) => {
  // Accept splits with a quantity — locationName is optional. addQty is already
  // in STOCKING units (the receive step converts a bulk line by units_per_pack).
...
export const fetchSupplierProfileById = async (supplierProfileId) => {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      storefront_contact_name, storefront_contact_role,
      storefront_contact_email, storefront_contact_phone
    `)
    .eq('id', supplierProfileId)
    .maybeSingle();
  if (error) throw error;
