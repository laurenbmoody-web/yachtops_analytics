...
export default function ReturnSlipPage() {
  ...
  const handleSubmitToSupplier = async () => {
    ...
    const vesselName = vessel?.vessel_type_label || 'Vessel';
    ...
    const itemLines = items.map(item => {
      const lines = [
        `• ${item.raw_name}`,
        `  Delivered: ${item.quantity ?? '—'}${item.unit ? ' ' + item.unit : ''} · Return Qty: ${item.return_qty ?? item.quantity}`,
        `  Reason: ${item.return_reason || 'Not ordered'}`,
      ];
      if (item.return_notes) lines.push(`  Notes: ${item.return_notes}`);
      return lines.join('\n');
    }).join('\n\n');

    const body = [
      '** Please attach the signed return slip PDF to this email before sending **',
      '',
      'RETURN SLIP',
      `${vesselName}${vessel?.imo_number ? ' · IMO: ' + vessel.imo_number : ''}`,
      `Date: ${today}`,
      preparedBy ? `Prepared by: ${preparedBy}` : null,
      '',
      `SUPPLIER: ${supplierInfo.name || '—'}`,
      supplierInfo.phone   ? `Phone: ${supplierInfo.phone}`   : null,
      supplierInfo.email   ? `Email: ${supplierInfo.email}`   : null,
      supplierInfo.address ? `Address: ${supplierInfo.address}` : null,
      '',
      orderMeta.ref  ? `Order Ref: ${orderMeta.ref}`   : null,
      orderMeta.date ? `Order Date: ${orderMeta.date}` : null,
      '',
      'ITEMS FOR RETURN:',
      '─'.repeat(40),
      itemLines,
      '─'.repeat(40),
      '',
      'Please confirm receipt of the returned items.',
      '',
      'This return slip was generated by Cargo (cargotechnology.app).',
      'A signed copy is attached.',
    ].filter(l => l !== null).join('\n');

    const mailto = `mailto:${supplierInfo.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
