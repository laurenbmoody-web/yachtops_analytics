...
import { UNIT_GROUPS, UNIT_GROUP_VALUES, STOCK_UNIT_GROUPS, STOCK_UNIT_VALUES, normalizeUnit } from '../../../data/unitGroups';
...
const CompoundMeasureCell = ({ item, editingCell, setEditingCell, onSave, onStep }) => {
  ...
  return (
    <div className="flex items-center gap-1 px-2 min-h-[38px]" onClick={e => e.stopPropagation()}>
      ...
      {/* Unit */}
      <select
        value={STOCK_UNIT_VALUES.has(normalizeUnit(item.unit)) ? normalizeUnit(item.unit) : (item.unit || 'each')}
        ...
        style={{ maxWidth: 58 }}
      >
        {item.unit && !STOCK_UNIT_VALUES.has(normalizeUnit(item.unit)) && <option value={item.unit}>{item.unit}</option>}
