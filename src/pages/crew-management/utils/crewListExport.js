...
const drawKeyGrid = (doc, pairs, x, y, colW, perRow, accent) => {
  ...
  // Return the last VALUE baseline (not a full trailing row) so the caller can
  // add the same gap below as sits above the grid — symmetric spacing.
  return y + (Math.ceil(pairs.length / perRow) - 1) * rowH + 4.6;
};
...
export const exportCrewListPDF = async (o) => {
  ...
  y = drawKeyGrid(doc, vPairs, M, y, gridColW, 6, labelColor) + 5;
  ...
  y = drawKeyGrid(doc, voyPairs, M, y, (pageW - 2 * M) / 5, 5, labelColor) + 5;
  ...
  // One page for crews up to 20 (shrink to fit); crews over 20 keep a
  // comfortable size and flow to a second page. The multi-word column headers
  // wrap to ~2 lines, so estimate the table as (N body + 2 header) text lines
  // at the real line-height (fs / 72 * 25.4 * lineFactor); the 1.04 fudge keeps
  // the actual render just inside the estimate so the footer never spills.
  const FOOTER_RESERVE = 50; // print-safe margin below the signature
  const availH = pageH - y - FOOTER_RESERVE;
  const estTableH = (fs, pd) =>
    ((rows.length + 2) * fs * 0.4057 + (rows.length + 1) * (2 * pd + 0.3)) * 1.04;
  let bodyFs = 7.6;
  let pad = editorial ? 2 : 1.8;
  if (rows.length <= 20) {
    while (bodyFs > 4.6 && estTableH(bodyFs, pad) > availH) {
      bodyFs -= 0.2;
      if (pad > 0.85) pad -= 0.06;
    }
  } else {
    bodyFs = 7; pad = editorial ? 1.8 : 1.6;
  }
  ...
  // ── Footer (flows under the table; the fit above guarantees a print-safe
  // margin so the master's name never lands on the trim edge) ────────────────
  let fy = (doc.lastAutoTable?.finalY || y) + 6;
  if (fy > pageH - 42) { doc.addPage(); fy = 22; }

  // Master's declaration (one line at full width).
  ...
  // Master name / signature / stamp (right).
  const sigW = 90;
