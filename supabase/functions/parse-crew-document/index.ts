...
const DOC_TYPE_IDS = [
  // travel & identity
  'passport', 'passport_certified_copy', 'national_id', 'driving_licence', 'seamans_book', 'tax_residency', 'visa_us_b1b2', 'visa_schengen', 'visa_other',
  // medical
...
- For ANY identity document (passport, passport_certified_copy, national_id, driving_licence, seamans_book), ALSO set details.holder_name to the full name of the person the document belongs to, exactly as printed (given names then surname, e.g. "ALEX MORGAN RIVERS"). This is the document holder, not any certifier/official.
- A passport photocopy stamped or endorsed "CERTIFIED TRUE COPY" (or similar) and signed by a professional (solicitor, notary, accountant, etc.) → passport_certified_copy, NOT passport. Put the certifier's profession in details.certifier_profession and the date of certification in details.certification_date ("YYYY-MM-DD"); the document_number is still the passport number, and the issuing_authority is the certifier's name.
...
- Where the document type has extra detail fields, fill the ones you can read using these exact keys/values (omit any you cannot read): medical (eng1/seafarer_medical) → result one of ["Fit","Fit with restrictions","Unfit"] plus restrictions text; gmdss → certificate_type one of ["GOC (General)","ROC (Restricted)"]; food_hygiene → level one of ["Level 1","Level 2","Level 3","Level 4"]; aec → level one of ["AEC 1","AEC 2","AEC 1 & 2"]; yachtmaster → grade one of ["Yachtmaster Coastal","Yachtmaster Offshore","Yachtmaster Ocean"]; helm_management → level one of ["Operational","Management"]; visas → entries one of ["Single","Multiple"] plus max_stay; tax_residency → country and tax_year; dive → agency one of ["PADI","SSI","BSAC","RAID","SDI","TDI","CMAS","NAUI"] plus level; wine_spirits → discipline one of ["Wine","Spirits","Sake"] plus level; driving_licence → country and categories (e.g. "B, BE"); coc also → limitations (tonnage/area as printed); yachtmaster/rya_day_skipper/rya_coastal_skipper → discipline one of ["Power","Sail"]; pwc_jetski → qualification_type one of ["Proficiency","Instructor"]; guest_interior → level. For a CoC, the 5-yearly revalidation date is the expiry_date.
