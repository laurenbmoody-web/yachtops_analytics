...
7. The NEW CLIENT REQUEST may be a short follow-up that only makes sense against the conversation — e.g. "add another 20", "same again", "6 more of those", "double the champagne". CRITICAL: each quote you produce is ADDED to the client's existing order when they accept it — it is NOT a restatement of the whole order. So:
   a. Include ONLY the item(s) THIS request is adding or changing. Do NOT re-list items from earlier quotes that this request doesn't mention — re-listing them would duplicate them on the order.
   b. Quote the quantity BEING ADDED, not a cumulative total: "add another 20" → qty 20 of that product (never 90). "6 more" → qty 6.
   c. Resolve which product they mean from CONVERSATION SO FAR / MOST RECENT QUOTE, and REUSE the price and unit that product already had in the most recent quote — never null out or change a price it already had.
   d. In quote_text, name the addition and, for context, the running total it brings them to (e.g. "Adding 20 × Veuve Clicquot — that brings your order to 90 bottles.").
   If the follow-up is genuinely ambiguous (nothing prior to attach it to), return an empty items array and use quote_text to ask a brief clarifying question naming what you need.
