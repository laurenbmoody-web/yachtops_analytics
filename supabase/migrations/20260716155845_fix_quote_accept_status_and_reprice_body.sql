-- Two fixes to the chat-quote flow:
--
-- 1. accept_supplier_quote inserted provisioning_items with status 'pending',
--    which isn't in provisioning_items_status_check (draft/ordered/received/
--    partial/not_received/returned) — so Accept 400'd. Priced lines now land
--    'ordered' (they're being bought), unpriced lines 'draft'.
--
-- 2. reprice_supplier_quote gains an optional p_body so the supplier's message
--    text can be refreshed when prices are added — clearing the stale
--    "unpriced / not in catalogue" note once the item is priced.
...
begin
  ...
  loop
    ...
    -- Mirror onto the crew's provisioning board. status must be one of the
    -- provisioning_items_status_check values: priced → 'ordered', else 'draft'.
...
-- Reprice now also accepts an optional new body so the quote text can be
-- refreshed alongside the prices. Drop the old 2-arg signature first so the
-- new default-param version doesn't create an ambiguous overload.
drop function if exists public.reprice_supplier_quote(uuid, jsonb);

create or replace function public.reprice_supplier_quote(p_message_id uuid, p_quote jsonb, p_body text default null)
returns void
language plpgsql
security definer
set search_path = public
...
grant execute on function public.reprice_supplier_quote(uuid, jsonb, text) to authenticated;
