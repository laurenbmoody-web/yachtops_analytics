-- Migration: Fix accept_crew_invite_v3 "Structure of query does not match function result type"
-- Root cause: The RETURNS TABLE declaration includes column names (tenant_id, role, role_id)
-- that are also field names on v_invite_record. PostgreSQL resolves these as the output
-- column variables (NULL at that point) rather than the record fields, causing a type
-- structure mismatch at runtime.
-- Fix: Extract all return values into explicitly-typed local variables BEFORE calling
-- RETURN QUERY SELECT, so there is zero ambiguity in the SELECT list.
-- Date: 2026-02-27

-- Drop existing v3 function
DROP FUNCTION IF EXISTS public.accept_crew_invite_v3(TEXT, TEXT);

-- Recreate with local variables to avoid output-column name collision
...
BEGIN
    ...
    INSERT INTO public.tenant_members (tenant_id, user_id, role, role_id, role_legacy, active, status)
    VALUES (
        v_out_tenant_id,
        v_user_id,
        v_out_role,
