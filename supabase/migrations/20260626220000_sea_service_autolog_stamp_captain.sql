-- Per-endorser testimonials need to know which master covered each day. Stamp the
-- vessel's captain (COMMAND user) onto each auto-logged day, so the Nautilus/PYA
-- export can split one document per command spell (per captain), scoped to that
-- captain's days only — never mixing other captains, manual entries or other ships.
alter table public.sea_service_entries add column if not exists master_user_id uuid;

-- (Function body kept in sync with the applied migration
-- 20260626220000_sea_service_autolog_stamp_captain.)
