-- Fix: a provisioning board auto-created when the crew accepts a chat quote
-- wasn't appearing on /provisioning, and the order had no board link.
--
-- Root cause: accept_supplier_quote created the board setting only created_by.
-- A board that's visible on the wall (matching every board the crew make by
-- hand — see provisioning/index.jsx) also needs owner_id, a real visibility,
-- and the owner's department_id. Without owner_id + visibility the board is
-- effectively private-with-no-owner, so the board-wall filter + the client
-- canViewList guard both hid it.
--
-- This redefines accept_supplier_quote to stamp the board the same way the UI
-- does: owner_id + created_by = the accepting member, visibility = 'department',
-- department_id = the member's department. It also backfills the boards already
-- created by the older RPC so the ones the crew already accepted show up.
--
-- Replaces accept_supplier_quote from 20260716170500 (logic otherwise identical).
...
begin
  ...
  if v_order is null then
    ...
    values
      (v_tenant, coalesce(v_sname, 'Supplier') || ' — chat request', 'draft', v_cur,
...
-- Backfill: boards the older RPC already created (created_by set, but owner_id
-- null / visibility private) so they surface on the wall now. Owner them to
-- their creator and give them department visibility, matching the new path.
update public.provisioning_lists
   set owner_id    = coalesce(owner_id, created_by),
       visibility  = 'department',
       is_private  = false
 where title like '% — chat request%'
   and (owner_id is null or coalesce(visibility, 'private') = 'private');
