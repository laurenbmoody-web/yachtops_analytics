-- Migration: Backfill team_jobs for existing rotation_assignments with linked_job_id = NULL
-- Also fixes sync_rotation_job to handle NULL p_assignment_id gracefully

-- ── Drop and recreate sync_rotation_job with improved NULL handling ──
...
BEGIN
  ...
  -- Also check by member + date + source if no assignment match
...
-- ── Backfill: Create team_jobs for rotation_assignments that have no linked_job_id ──
-- Only backfill assignments from today onwards (past assignments are historical)
DO $$
DECLARE
  r RECORD;
  v_job_id UUID;
