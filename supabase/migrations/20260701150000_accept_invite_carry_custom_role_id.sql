-- Push Role / Department / Permission through to the crew profile on join.
--
-- accept_crew_invite_v3 already writes role_id, department_id and permission_tier
-- to tenant_members (and the crew-profile page reads all three from there), so a
-- STANDARD-role invite already lands correctly. But the INSERT omitted
-- custom_role_id — so an invite created via the "Other…" custom-role path joined
-- with a NULL role: the profile shows no role title because roleTitle resolves to
-- custom_role.name || roles.name and neither was set.
--
-- Fix: carry custom_role_id from the invite into tenant_members, and when the
-- permission tier wasn't set explicitly, fall back to the custom role's default
-- tier as well as the standard role's.
...
BEGIN
    ...
    -- custom_role_id is carried through so custom-role invites show a role title.
