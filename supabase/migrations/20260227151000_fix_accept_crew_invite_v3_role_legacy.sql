-- Migration: Fix accept_crew_invite_v3 missing role_legacy column
-- Root cause: INSERT into tenant_members was missing role_legacy column which has a NOT NULL constraint.
-- Fix: Add role_legacy to the INSERT, using the invite's invited_role value.
-- Date: 2026-02-27

-- Drop existing v3 function
DROP FUNCTION IF EXISTS public.accept_crew_invite_v3(TEXT, TEXT);

-- Recreate with role_legacy included in INSERT
...
BEGIN
    ...
    -- Create tenant membership with role_id AND role_legacy
    -- role_legacy is a NOT NULL column — populate it with the invite's role value
