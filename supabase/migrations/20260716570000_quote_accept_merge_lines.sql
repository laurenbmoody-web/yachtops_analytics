-- Consolidate quantities instead of stacking duplicate lines.
--
-- "add another 20 Veuve" should take the order's Veuve line from 70 → 90, one
-- line — not add a second 20-line. So when a chat quote commits an item, it now
-- MERGES into a matching line (same product + unit + price) on the order and the
-- board, incrementing the quantity; only a genuinely new item makes a new line.
--
-- Gating interaction (per the agreed model): an over-threshold (held) item is
-- NOT put on the board on accept — it's recorded as a pending order line for the
-- approver to see, and merges into the board line only when it's signed off.
--
-- Re-emits accept_supplier_quote (item handling) and decide_supplier_order_approval
-- (approval commits + merges the held lines). Everything else is unchanged.
...
begin
  ...
  loop
    ...
    -- Priced + not gated → commit now. Gated (or unpriced) → held.
    v_commit := (v_price is not null) and not v_gated;

    if v_commit then
      -- Order line: merge into a matching confirmed line, else insert.
      update public.supplier_order_items
         set quantity = quantity + v_qty
       where order_id = v_order and status = 'confirmed'
         and lower(coalesce(item_name, '')) = lower(v_name)
         and coalesce(unit, '') = coalesce(v_unit, '')
         and coalesce(agreed_price, quoted_price, estimated_price) is not distinct from v_price;
      if not found then
      ...
      -- Board line: merge into a matching ordered line, else insert.
      if v_list is not null then
        ...
        if not found then
          ...
          values (v_list, v_name, v_qty, v_unit, v_price, 'manual', 'ordered', 'Added from a supplier chat quote');
        end if;
      end if;
    else
      -- Held (gated) or unpriced: record a pending order line so the approver
      -- sees it + the total. A gated item stays OFF the board until sign-off;
      -- an ungated unpriced item still gets a draft board line (price TBC).
      insert into public.supplier_order_items
      ...
      if v_list is not null and not v_gated then
        insert into public.provisioning_items
          (list_id, name, quantity_ordered, unit, estimated_unit_cost, source, status, notes)
        values (v_list, v_name, v_qty, v_unit, v_price, 'manual', 'draft', 'Added from a supplier chat quote');
      end if;
    end if;
...
-- On sign-off, COMMIT the held lines and MERGE them into the order/board (so an
-- approved addition folds into the existing line rather than a separate one).
...
begin
  ...
  if p_approved then
    -- Each held (pending) line: merge into a matching confirmed order line if
    -- one exists (and drop the held row), else confirm it in place. Then merge
    -- it onto the board.
    for v_held in
    ...
    loop
      ...
      update public.supplier_order_items
         set quantity = quantity + v_held.quantity
       where order_id = p_order_id and id <> v_held.id and status = 'confirmed'
         and lower(coalesce(item_name, '')) = lower(coalesce(v_held.item_name, ''))
         and coalesce(unit, '') = coalesce(v_held.unit, '')
         and coalesce(agreed_price, quoted_price, estimated_price) is not distinct from v_price;
      if found then
        delete from public.supplier_order_items where id = v_held.id;
      else
        update public.supplier_order_items
