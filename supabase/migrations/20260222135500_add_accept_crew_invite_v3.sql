-- Migration: Add accept_crew_invite_v3 RPC with tenant_id in JSON response
-- Created: 2026-02-22
-- Purpose: Replace v2 with v3 that returns tenant_id in response for polling logic

-- Drop existing v3 function if it exists
DROP FUNCTION IF EXISTS public.accept_crew_invite_v3(TEXT, TEXT);

-- Create new versioned RPC function to accept crew invite with tenant_id in response
